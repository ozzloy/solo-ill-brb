...
if [ "$status_code" != "$expected_status_code" ]; then
    echo "status code is $status_code, expected $expected_status_code"
    exit 1
fi
...
body=$(echo "$body" | jq -c '.')
normalized_expected_body=$(echo "$expected_body" | jq -c '.')
if [ "$body" != "$normalized_expected_body" ]; then
